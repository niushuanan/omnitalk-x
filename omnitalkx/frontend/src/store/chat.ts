...
interface ChatStore {
    ...
    switchToSession: (sessionName: string) => void;
    ...
    addMessage: (sessionName: string, message: ChatMessage) => void;
    ...
    updateMessageById: (sessionName: string, messageId: number, updater: Partial<ChatMessage>) => void;
    ...
    clearCurrentSession: (sessionName: string) => void;
    ...
    persist(
        (set, get) => ({
            ...
            switchToSession(sessionName) {
                const sessions = get().sessions;
                let targetIdx = sessions.findIndex((session) => session.name === sessionName);
                if (targetIdx < 0) {
                    const session = createSession({
                        name: sessionName,
                        bot: sessionName,
                    ...
                    sessions.push(session);
                    targetIdx = sessions.length - 1;
                }
                set({ 
                    currentSessionIndex: targetIdx,
            ...
            addMessage(sessionName: string, message: ChatMessage) {
                const sessions = get().sessions;
                const targetIdx = sessions.findIndex((session) => session.name === sessionName);
                if (targetIdx >= 0) {
                    sessions[targetIdx].messages.push(message);
            ...
            updateMessageById(sessionName, messageId, updater) {
                const session = get().sessions.find(s => s.name === sessionName);
                if (!session) return;
            ...
            clearCurrentSession(sessionName: string) {
                ...
                const targetIdx = sessions.findIndex((session) => session.name === sessionName);
                if (targetIdx >= 0) {
                    sessions[targetIdx].messages = [];
                    sessions[targetIdx].clearContextIndex = 0;
                    set(() => ({ sessions: [...sessions] }));
                }
                // 清除对应的上下文本地存储
